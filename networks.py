...
def test_net(width, height, lr, logging, conv1, conv2, conv3):
...
#NETWORKS IN DEPLOYMENT

def net_v0_0_1(width, height, lr, logging):
    network = input_data(shape=[None, width, height, 1], name='input')
    network = conv_2d(network, 32, 4, strides=2, activation='relu', name='conv_5x5_1')
    network = max_pool_2d(network, 3, strides=2, name='pool_1')
    network = local_response_normalization(network)
    
    network = conv_2d(network, 32, 3, strides=1, activation='relu', name='conv_3x3_1')
    network = max_pool_2d(network, 3, strides=1, name='pool_2')
    network = local_response_normalization(network)
    
    network = conv_2d(network, 32, 4, strides=2, activation='relu', name='conv_3x3_2')
    network = max_pool_2d(network, 3, strides=2, name='pool_3')
    network = local_response_normalization(network)
    ...
    description = "Convolution Layer Setup: 4 x 3 x 4"
